...
def _render_sidebar() -> dict:
    ...
    with st.sidebar:
        ...
        if data_source == "📂 Folder Scan":
            # Default to the sample resumes folder relative to app.py
            default_folder = str(Path(__file__).parent / "assets" / "input_and_output" / "sample_resumes")
...
def _render_candidate_selection(
    ...
    with col_export2:
        if st.button("💬 Save Chat Record", use_container_width=True):
            with st.spinner("Saving chat transcript to Google Sheet..."):
                try:
                    creds_path = st.session_state.get("credentials_path", "credentials.json")
                    export_chat_to_sheet(
                        credentials_path=creds_path,
